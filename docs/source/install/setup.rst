...
needed for attaching the silverbox to the mount.

All other components are located diectly on the telescope, so the is only an
ethernet connection and two 12 V power supply wires to the telescope.

.. image:: image/setup.drawio.svg
    :align: center