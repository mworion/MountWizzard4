...
class MeasureWindow(toolsQtWidget.MWidget):
    ...
    def plotRa(self):
        ...
        timeT = (time_ticks + time_end)
        timeL = [x.astype(dt).strftime('%H:%M:%S') for x in timeT]
        timeT = timeT.astype('datetime64[s]').astype('int')

        x = data['time'][start:-1:cycle].astype('datetime64[s]').astype('int')
        y = data['directWeatherTemp'][start:-1:cycle]

        print(len(y), y)
        self.plot.setData(x=x, y=y)
        self.ui.measure.getAxis('bottom').setTicks([list(zip(timeT, timeL))])
        return True
    ...
    def drawMeasure(self):
        ...
        self.plot = pg.PlotDataItem(pen=self.M_BLUE)
        self.ui.measure.addItem(self.plot)
        self.plotRa()
