...
# Remarks from discussions about the correct transformation in the 10micron forum:
...
#
# NOVAS is "correct" in the sense that it computes a correct precession, but does
# not account for nutation, so it isn't "correct" in the sense that it is the right
# transformation to use. If you take the NOVAS result and apply nutation/light
# aberration, you should obtain the right transformation.
#
...
#
# There are some subtle effects of less than one arcsecond that aren't worth
# modelling (such as diurnal aberration for example), but these can be the source
# of small differences between different algorithms.
#
# The algorithms used in the mount don't use the CIRS (celestial intermediate
# reference system), but use what is defined as "classical geocentric apparent
# place" in the SOFA AstrometryTools "Quick start" section. You obtain the correct
# coordinates by using iauAtci13. Residual (and negligible) differences are due to
# the mount algorithms being less refined than those used in SOFA.
