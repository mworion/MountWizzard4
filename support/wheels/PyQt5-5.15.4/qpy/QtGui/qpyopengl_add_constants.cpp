/*
 * This implements a function to populate a Python object with the OpenGL
...
static OpenGLEnum_i openGLEnums_i[] = {
    ...
    {"GL_ALL_ATTRIB_BITS", static_cast<int>(0xFFFFFFFF)},
    {"GL_COVERAGE_BUFFER_BIT_NV", 0x00008000},
    {"GL_CLIENT_PIXEL_STORE_BIT", 0x00000001},
    {"GL_CLIENT_VERTEX_ARRAY_BIT", 0x00000002},
    {"GL_CLIENT_ALL_ATTRIB_BITS", static_cast<int>(0xFFFFFFFF)},
    ...
    {"GL_ALL_BARRIER_BITS", static_cast<int>(0xFFFFFFFF)},
    {"GL_ALL_BARRIER_BITS_EXT", static_cast<int>(0xFFFFFFFF)},
    ...
    {"GL_ALL_SHADER_BITS", static_cast<int>(0xFFFFFFFF)},
    {"GL_ALL_SHADER_BITS_EXT", static_cast<int>(0xFFFFFFFF)},
    ...
    {"GL_TEXCOORD4_BIT_PGI", static_cast<int>(0x80000000)},
    ...
    {"GL_MULTISAMPLE_BUFFER_BIT7_QCOM", static_cast<int>(0x80000000)},
...
/*
 * Add the OpenGL constants to a Python object.  Return -1 and raise an
 * exception on error.
 */
int qpyopengl_add_constants(PyObject *obj)
{
    ...
    for (i = 0; openGLEnums_i[i].name; ++i)
    {
#if PY_MAJOR_VERSION >= 3
        py_value = PyLong_FromLong(openGLEnums_i[i].value);
#else
        py_value = PyInt_FromLong(openGLEnums_i[i].value);
#endif

        if (!py_value)
            return -1;

        rc = PyObject_SetAttrString(obj, openGLEnums_i[i].name, py_value);
        Py_DECREF(py_value);
    ...
    for (i = 0; openGLEnums_u[i].name; ++i)
    {
        py_value = PyLong_FromUnsignedLong(openGLEnums_u[i].value);

        if (!py_value)
            return -1;

        rc = PyObject_SetAttrString(obj, openGLEnums_u[i].name, py_value);
        Py_DECREF(py_value);
    ...
    for (i = 0; openGLEnums_ull[i].name; ++i)
    {
        py_value = PyLong_FromUnsignedLongLong(openGLEnums_ull[i].value);

        if (!py_value)
            return -1;

        rc = PyObject_SetAttrString(obj, openGLEnums_ull[i].name, py_value);
        Py_DECREF(py_value);
