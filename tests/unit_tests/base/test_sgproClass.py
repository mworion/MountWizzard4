...
def test_createDevice_empty_response(function):
...
def test_createDevice_with_spaces(function):
...
def test_enumerateDevices_success(function):
...
def test_enumerateDevices_no_devices(function):
...
def test_enumerateDevices_empty_response(function):
...
def test_pollDeviceStatus_connected_to_disconnected(function):
...
def test_pollDeviceStatus_disconnected_to_connected(function):
...
def test_pollDeviceStatus_already_connected(function):
...
def test_pollDeviceStatus_none_response(function):
...
def test_callDeviceMethod(function):
    """Test direct endpoint call via callDeviceMethod."""
    with mock.patch.object(function, "requestProperty") as mock_request:
        mock_request.return_value = {"Success": True}
        result = function.callDeviceMethod("image", BinningMode=1)
        assert result == {"Success": True}
        mock_request.assert_called_once_with("image", params={"BinningMode": 1})
...
def test_runnerCommunicationLoop_connected_state(function):
    ...
    function.UPDATE_RATE = 0.25
    with (
        mock.patch.object(function, "pollDeviceStatus") as mock_poll_status,
        ...
        with contextlib.suppress(KeyboardInterrupt):
            function.runnerCommunicationLoop()
        mock_poll_status.assert_called()
        mock_poll_data.assert_called()
...
def test_discoverDevices_camera(function):
    ...
    devices = ["Camera1", "Camera2"]
    with mock.patch.object(function, "enumerateDevices") as mock_enum:
...
def test_createDevice_url_encoding(function):
