...
def test_setAlpacaServer_1(function):
    """Test setAlpacaServer with valid server name."""
    function.ui.alpacaServerList.clear()
    function.ui.alpacaServerList.addItem("192.168.1.100:11111")
    function.ui.alpacaHostAddress.clear()
    function.ui.alpacaPort.clear()
    function.setAlpacaServer()
    assert function.ui.alpacaHostAddress.text() == "192.168.1.100"
    assert function.ui.alpacaPort.text() == "11111"


def test_setAlpacaServer_2(function):
    """Test setAlpacaServer with different server name."""
    function.ui.alpacaServerList.clear()
    function.ui.alpacaServerList.addItem("localhost:8888")
    function.ui.alpacaHostAddress.clear()
    function.ui.alpacaPort.clear()
    function.setAlpacaServer()
    assert function.ui.alpacaHostAddress.text() == "localhost"
    assert function.ui.alpacaPort.text() == "8888"


def test_updateServerList_1(function):
    """Test updateServerList with multiple servers."""
    function.discovers["alpaca"]["serverList"] = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].clear = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].setView = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].addItem = mock.MagicMock()
    
    serverNames = ["192.168.1.100:11111", "192.168.1.101:11111"]
    function.updateServerList("alpaca", serverNames)
    
    function.discovers["alpaca"]["serverList"].clear.assert_called_once()
    assert function.discovers["alpaca"]["serverList"].addItem.call_count == 2


def test_updateServerList_2(function):
    """Test updateServerList with single server."""
    function.discovers["alpaca"]["serverList"] = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].clear = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].setView = mock.MagicMock()
    function.discovers["alpaca"]["serverList"].addItem = mock.MagicMock()
    
    serverNames = ["192.168.1.100:11111"]
    function.updateServerList("alpaca", serverNames)
    
    function.discovers["alpaca"]["serverList"].clear.assert_called_once()
    function.discovers["alpaca"]["serverList"].addItem.assert_called_once()


def test_discoverServers_1(function):
    """Test discoverServers with servers found."""
    alpacaInstance = mock.MagicMock()
    alpacaInstance.discoverServers = mock.MagicMock(
        return_value=["192.168.1.100:11111", "192.168.1.101:11111"]
    )
    function.app.dReg["telescope"].run["alpaca"] = alpacaInstance
    function.discovers["alpaca"]["serverList"] = mock.MagicMock()
    
    with mock.patch("mw4.gui.extWindows.devicePopupW.changeStyleDynamic"):
        with mock.patch.object(function, "updateServerList") as mock_update:
            function.discoverServers()
            mock_update.assert_called_once_with(
                "alpaca", ["192.168.1.100:11111", "192.168.1.101:11111"]
            )


def test_discoverServers_2(function):
    """Test discoverServers with no servers found."""
    alpacaInstance = mock.MagicMock()
    alpacaInstance.discoverServers = mock.MagicMock(return_value=[])
    function.app.dReg["telescope"].run["alpaca"] = alpacaInstance
...
def test_discoverServers_3(function):
    """Test discoverServers with no message emissions for empty result."""
    alpacaInstance = mock.MagicMock()
    alpacaInstance.discoverServers = mock.MagicMock(return_value=[])
    function.app.dReg["telescope"].run["alpaca"] = alpacaInstance
    
    with (
        mock.patch("mw4.gui.extWindows.devicePopupW.changeStyleDynamic"),
        mock.patch.object(function, "updateServerList") as mock_update,
    ):
        function.discoverServers()
        # Verify that updateServerList is not called when no servers found
        mock_update.assert_not_called()
